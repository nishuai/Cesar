...
#setwd('E:/wrk/CESAR_cnv/')
#args=c('E:/wrk/CESAR_cnv/inputdata/segmented_bed_30k.bed', 
#       'E:/wrk/CESAR_cnv/0715_test/met2.1875ERBB2.625//',
#       'E:/wrk/CESAR_cnv/temp/')
...
print('Finding the best anchor number for each segment with minimal sd')
gene_anchor_sd=list()
for (n_anchors in 9:20){
sds=c()
for (segment in c(128:132, 191:193, 124:127)){
top_I=tail(order(cor(mean_coorelations)[segment,]), n_anchors)
top_I=top_I[!is.na(cor(mean_coorelations)[segment,][top_I])]
top_I=top_I[-length(top_I)]
print(top_I)
###remove anchors with ratio too large or small 
I=colMeans(mean_coorelations[,top_I])/mean(mean_coorelations[,segment])<0.5 |
            colMeans(mean_coorelations[,top_I])/mean(mean_coorelations[,segment])>2
top_I=top_I[!I]

###calculate normal sd
top_normal=fitdistr(
  rowMeans(mean_coorelations[,top_I])/mean_coorelations[,segment],'normal')
top_normal=top_normal$estimate
top_normal=top_normal[2]/top_normal[1]
sds=c(sds, top_normal)
}
gene_anchor_sd[[n_anchors-8]]=sds
}

gene_anchor_sd=do.call(rbind, gene_anchor_sd)
sd_min_anchor=apply(gene_anchor_sd, 2, function(x) which.min(x))
sd_min_anchor=sd_min_anchor+8

###Use the best anchor number to generate the model 
print('Use the best anchor number to generate the model')

met_top_I=tail(order(cor(mean_coorelations)[128,]), sd_min_anchor[1])
met_top_I=met_top_I[!is.na(cor(mean_coorelations)[128,][met_top_I])]
met_top_I=met_top_I[-length(met_top_I)]

met_top_I2=tail(order(cor(mean_coorelations)[129,]), sd_min_anchor[2])
met_top_I2=met_top_I2[!is.na(cor(mean_coorelations)[129,][met_top_I2])]
met_top_I2=met_top_I2[-length(met_top_I2)]

met_top_I3=tail(order(cor(mean_coorelations)[130,]), sd_min_anchor[3])
met_top_I3=met_top_I3[!is.na(cor(mean_coorelations)[130,][met_top_I3])]
met_top_I3=met_top_I3[-length(met_top_I3)]

met_top_I4=tail(order(cor(mean_coorelations)[131,]), sd_min_anchor[4])
met_top_I4=met_top_I4[!is.na(cor(mean_coorelations)[131,][met_top_I4])]
met_top_I4=met_top_I4[-length(met_top_I4)]

met_top_I5=tail(order(cor(mean_coorelations)[132,]), sd_min_anchor[5])
met_top_I5=met_top_I5[!is.na(cor(mean_coorelations)[132,][met_top_I5])]
met_top_I5=met_top_I5[-length(met_top_I5)]

###ERBB2 anchor
erbb_top_I=tail(order(cor(mean_coorelations)[191,]), sd_min_anchor[6])
erbb_top_I=erbb_top_I[!is.na(cor(mean_coorelations)[191,][erbb_top_I])]
erbb_top_I=erbb_top_I[-length(erbb_top_I)]

erbb_top_I2=tail(order(cor(mean_coorelations)[192,]), sd_min_anchor[7])
erbb_top_I2=erbb_top_I2[!is.na(cor(mean_coorelations)[192,][erbb_top_I2])]
erbb_top_I2=erbb_top_I2[-length(erbb_top_I2)]

erbb_top_I3=tail(order(cor(mean_coorelations)[193,]), sd_min_anchor[8])
erbb_top_I3=erbb_top_I3[!is.na(cor(mean_coorelations)[193,][erbb_top_I3])]
erbb_top_I3=erbb_top_I3[-length(erbb_top_I3)]
###EGFR anchor

egfr1_top_I=tail(order(cor(mean_coorelations)[124,]), sd_min_anchor[9])
egfr1_top_I=egfr1_top_I[!is.na(cor(mean_coorelations)[124,][egfr1_top_I])]
egfr1_top_I=egfr1_top_I[!egfr1_top_I %in% c(124:127)]

egfr2_top_I=tail(order(cor(mean_coorelations)[125,]), sd_min_anchor[10])
egfr2_top_I=egfr2_top_I[!is.na(cor(mean_coorelations)[125,][egfr2_top_I])]
egfr2_top_I=egfr2_top_I[!egfr2_top_I %in% c(124:127)]

egfr3_top_I=tail(order(cor(mean_coorelations)[126,]), sd_min_anchor[11])
egfr3_top_I=egfr3_top_I[!is.na(cor(mean_coorelations)[126,][egfr3_top_I])]
egfr3_top_I=egfr3_top_I[!egfr3_top_I %in% c(124:127)]

egfr4_top_I=tail(order(cor(mean_coorelations)[127,]), sd_min_anchor[12])
...
print('generating models for possible snv segments')
